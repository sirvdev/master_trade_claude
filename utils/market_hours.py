...
def _mt5_symbol(symbol: str) -> str:
    """Normalise symbol to MT5 format while preserving the USDm suffix.
    ...
    upper = cleaned.upper()
    if upper.endswith("M") and len(upper) > 1:
        return upper[:-1] + "m"
    return upper
...
class MarketHoursChecker:
    ...
    def _load_fallback(self, symbol: str, mt5_sym: str) -> None:
        ...
        if not raw and mt5_sym.endswith("m"):
            raw = _FALLBACK_SESSIONS.get(mt5_sym[:-1])
        if not raw:
            logger.warning(
