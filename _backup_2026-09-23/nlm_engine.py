...
# Gold-only engine — but we keep the set so the threshold logic mirrors siblings.
...
class NLMStrategyEngine:
    ...
    def __init__(self, config: Dict):
        ...
        self.nlm_config = config.get('nlm', {})  # Optional NLM-specific overrides
        ...
        try:
            ...
            for tf, df in multi_tf_data.items():
                if df is None or len(df) < 50:
                    continue
                analysis['timeframe_snapshots'][tf] = self._build_nlm_snapshot(df, tf)
    ...
    def _build_nlm_snapshot(self, df: pd.DataFrame, tf: str) -> Dict:
        ...
        snap['sweep'] = self._detect_inducement_sweep(df, snap['liquidity_map'],
                                                     ind['atr']['current'])
        snap['mss'] = self._detect_market_structure_shift(df, swing_highs, swing_lows)
        ...
        for i in range(2, n - 3):
            ...
            if is_bearish and move_up > 1.5 * atr:
                obs.append({
                    ...
                    'mitigated': float(df['low'].iloc[i + 1:].min()) < float(c[i])
                    if i + 1 < n else False,
                })
            if is_bullish and move_down > 1.5 * atr:
                obs.append({
                    ...
                    'mitigated': float(df['high'].iloc[i + 1:].max()) > float(c[i])
                    if i + 1 < n else False,
                })

        # Keep only recent + unmitigated OBs
        recent = [ob for ob in obs if ob['index'] >= n - 40 and not ob['mitigated']]
        return recent[-6:]
    ...
    def _build_liquidity_map(self, df: pd.DataFrame,
                             swing_highs: List, swing_lows: List, tf: str) -> Dict:
        """
        ...
        cp = float(df['close'].iloc[-1])
        atr = self.indicators.calculate_atr(df)['current']
        eq_tol = max(cp * 0.0005, 0.1 * atr)  # 0.05% of price OR 10% of ATR
    ...
    def _detect_market_structure_shift(self, df: pd.DataFrame,
        ...
        # Most recent opposing swing point determines CHoCH potential
        # If the last structure point is a low and we break above the last high → bullish CHoCH
        # If the last structure point is a high and we break below the last low → bearish CHoCH
        if last_sl_idx > last_sh_idx and cp > last_sh_val:
            # Recent leg made a low; we now broke the prior high → CHoCH up
            return {'detected': True, 'type': 'CHoCH', 'direction': 'bullish',
                    'broken_level': last_sh_val}
        if last_sh_idx > last_sl_idx and cp < last_sl_val:
            return {'detected': True, 'type': 'CHoCH', 'direction': 'bearish',
                    'broken_level': last_sl_val}

        # BOS = continuation. If we have HH/HL and break the last high → bullish BOS
        hh = last_sh_val > prev_sh_val
        hl = last_sl_val > prev_sl_val
        lh = last_sh_val < prev_sh_val
        ll = last_sl_val < prev_sl_val

        if hh and hl and cp > last_sh_val:
            return {'detected': True, 'type': 'BOS', 'direction': 'bullish',
                    'broken_level': last_sh_val}
        if lh and ll and cp < last_sl_val:
    ...
    def _evaluate_nlm_setup(self, analysis: Dict, multi_tf_data: Dict,
        ...
        if self._is_gold_session_active():
        ...
        if score < threshold:
            out['entry_reason'] = f'NLM: confluence score {score} below threshold {threshold}'
            phase_results['final'] = f'FAIL — score {score}/{threshold}'
            out['nlm_phase_results'] = phase_results
            return out

        phase_results['final'] = f'PASS — score {score}'
    ...
    def _calc_nlm_sl(self, analysis: Dict, entry_price: float, atr: float) -> float:
        ...
        max_sl_atr_mult = float(self.nlm_config.get('max_sl_atr_multiplier', 2.5))
    ...
    def _get_threshold(self, symbol: str) -> int:
        # Per-symbol override must be NLM-specific ('nlm_confluence_threshold').
        # The generic 'confluence_threshold' in symbol config was tuned for the
        # classic engine (values 5-6); NLM scores pass 7 on Phase 1 alone, so
        # inheriting those values made this gate a no-op. Use nlm: config here.
        sc = self.config.get('symbols', {}).get(symbol, {})
        if 'nlm_confluence_threshold' in sc:
            return int(sc['nlm_confluence_threshold'])
        # B1: prefix match, so 'XAU/USDm' is recognised as XAUUSD.
        ms = self._norm_symbol(symbol)
        if any(ms.startswith(self._norm_symbol(v)) for v in _VOLATILE_SYMBOLS):
            return int(self.nlm_config.get('confluence_required', 12))
        return int(self.nlm_config.get('confluence_threshold_calm', 9))

    def _is_gold_session_active(self) -> bool:
        """Gold-specific session window check — London or NY open."""
        hour = datetime.utcnow().hour
        for name in ('london_open', 'ny_open'):
            start, end = _NLM_SESSIONS[name]
            if start <= hour < end:
                return True
        return False
    ...
    def _apply_nlm_filters(self, analysis: Dict, snaps: Dict,
        ...
        min_adx = float(self.nlm_config.get('min_htf_adx', 18))
        ...
        min_atr = float(self.nlm_config.get('min_atr_threshold',
